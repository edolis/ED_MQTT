...
void MqttClient::scheduleReconnect(uint32_t delay_ms) {
    if (mqtt_reconnect_timer == nullptr) {
        mqtt_reconnect_timer = xTimerCreate("mqtt_reconnect", pdMS_TO_TICKS(delay_ms),
                                            pdFALSE, nullptr, mqtt_reconnect_timer_cb);
...
// ── Event handler ──────────────────────────────────────────────────────
void MqttClient::handleEvent(esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    switch (event_id) {
        ...
        case MQTT_EVENT_DATA: {
            ...
            bool complete = (s_payload_expected > 0) ? (s_payload_len >= s_payload_expected)
            ...
            if (complete) {
                ...
                for (uint8_t i = 0; i < data_callback_count; ++i)
                    if (data_callbacks[i]) data_callbacks[i](event->client, event->topic,
                                event->topic_len, s_payload_buf, s_payload_len, msgID);
                s_payload_len = 0;
                s_payload_expected = 0;
            }
            xSemaphoreGive(get_mqtt_mutex());
...
void SAMPLE_derivedMqttClient::handleEvent(esp_event_base_t base,
    ...
    switch (event_id) {
        ...
        case MQTT_EVENT_DATA:
            ...
            break;
        default: MqttClient::handleEvent(base, event_id, event_data);
    }
}
